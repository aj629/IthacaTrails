...
def get_trails():
    ...
    # if name and difficulty and length and activity:
    #     trails = trails.query.filter(
    #         trails.name==name,trails.difficulty==difficulty,trails.length_range==length,
    #         trails.activities.any(name=activity)).all()   

    # if name and difficulty and length:
    #     trails = trails.query.filter(
    #         trails.name==name,trails.difficulty==difficulty,trails.length_range==length,).all()

    # if name and difficulty and activity:
    #     trails = trails.query.filter(
    #         trails.name==name,trails.difficulty==difficulty,
    #         trails.activities.any(name=activity)).all()  

    # if name and length and activity:
    #     trails = trails.query.filter(
    #         trails.name==name,trails.length_range==length,
    #         trails.activities.any(name=activity)).all()     

    # if difficulty and length and activity:
    #     trails = trails.query.filter(
    #         trails.difficulty==difficulty,trails.length_range==length,
    #         trails.activities.any(name=activity)).all()      
  
    # if name and difficulty:
    #     trails = trails.query.filter(trails.name==name,trails.difficulty==difficulty).all()
        
    # if name and length:
    #     trails = trails.query.filter_by(name=name, length_range=length).all()
    
    # if name and activity:
    #     trails = trails.query.filter(trails.name==name,trails.activities.any(name=activity)).all()

    # if difficulty and length:
    #     trails = trails.query.filter_by(difficulty==difficulty, length_range=length).all()
    
    # if difficulty and activity:
    #     trails = trails.query.filter(trails.difficulty==difficulty, trails.activities.any(name= activity).all())
    
    # if length and activity:
    #     trails = trails.query.filter(trails.length_range==length,trails.activities.any(name= activity).all()).all()

    # if name:
    #     trails = trails.query.filter_by(name=name).all()
    
    # if difficulty:
    #     trails = trails.query.filter_by(difficulty==difficulty).all()
    
    # if activity:
    #     trails = trails.query.filter(trails.activities.any(name=activity)).all()
    
    # if length:
    #     trails = trails.query.filter_by(length_range=length).all()
#--------------------------old conditionals-----------------------------
    # if name:
    #     trails = trails.query.filter_by(name=name).all()
    # if difficulty:
    #     trails = trails.query.filter_by(difficulty=difficulty).all()
    # #account for association table
    # if activity:
    #     trails = trails.query.filter(trails.activities.any(name=activity)).all()
    #     #https://stackoverflow.com/questions/40699642/how-to-query-many-to-many-sqlalchemy
    #     #Artist.query.filter(Artist._albums.any(id=album1.id)).all()
    # #can't be a one to one query - 4 translates to >=2
    # if length:
    #     trails = trails.query.filter_by(length_range=length).all()
...
### additionals routes to get trail by specific characteristics?? Same question as the additional reviews route
## look at get_reviews route for more info.
## LOOK INTO FILTER FOR SQLALCHEMY
...
#Reminder: switch the port after the move off the local server
if __name__ == "__main__":
    #loads in trail data
    #load_trail_data(app, db)
    app.run(host="0.0.0.0", port=5000)

#ASK ABOUT debug = True !!!!!!
